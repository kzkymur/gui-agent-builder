...
export type EngineState = {
  ...
  traceFinish: (traceId: string, patch: Partial<Pick<TraceNode, 'endedAt'|'output'|'error'>>) => void;
...
export const useEngineStore = create<EngineState>((set) => ({
  ...
  trace: { nodes: {}, roots: [] },
  resetRun: (runId) => set(() => ({
    run: { runId: runId ?? String(Date.now()), status: 'running', startedAt: Date.now(), errorCount: 0 },
    activeRunning: new Map(),
    inputBufByNode: {},
    latestInputByNode: {},
    latestOutputByNode: {},
  ...
  traceStart: (nodeId, parentId) => {
    ...
    set((s) => {
      ...
      if (parentId && nodes[parentId]) nodes[parentId] = { ...nodes[parentId], children: [...nodes[parentId].children, id] };
      return { trace: { nodes, roots } } as Partial<EngineState>;
    });
    return id;
  },
  traceFinish: (traceId, patch) => set((s) => {
    const node = s.trace.nodes[traceId];
    if (!node) return {} as Partial<EngineState>;
    return { trace: { nodes: { ...s.trace.nodes, [traceId]: { ...node, ...patch } }, roots: s.trace.roots } } as Partial<EngineState>;
  }),
  setInputBuf: (nodeId, input) => set((s) => ({ inputBufByNode: { ...s.inputBufByNode, [nodeId]: input } })),
  setLatestInput: (nodeId, input) => set((s) => ({ latestInputByNode: { ...s.latestInputByNode, [nodeId]: input } })),
  setLatestOutput: (nodeId, output) => set((s) => ({ latestOutputByNode: { ...s.latestOutputByNode, [nodeId]: output } })),
  markCompleted: (ok) => set((s) => ({ run: { ...s.run, status: ok ? 'completed' : 'failed', endedAt: Date.now() } })),
}));
