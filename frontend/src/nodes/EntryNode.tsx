...
export default function EntryNode({ data }: RFNodeProps<EntryData>) {
  ...
  return (
    <NodeChrome
      ...
      handles={
        <>
          <div className="node__handles-left" style={{ display: 'flex', alignItems: 'center', paddingLeft: 4 }}>
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
                const evt = new CustomEvent('engine:ignite', { detail: { entryId: (e.currentTarget as any).closest('[data-id]')?.getAttribute('data-id') ?? null } });
          ...
          <div className="node__handles-right">
            {items.map((it, idx) => (
              <div key={it.key + idx} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span title={it.value} style={{ fontSize: 11, color: '#9ca3af' }}>{it.key || `out-${idx}`}</span>
