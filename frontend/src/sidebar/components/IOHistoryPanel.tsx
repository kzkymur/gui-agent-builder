...
import { listNodeIO, type NodeIORecord } from "../../db/sqlite";
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {rows.map((r) => (
          <details key={r.id} className="io-item" style={{ border: "1px solid var(--gray-6)", borderRadius: 6, padding: 8, width: "100%" }}>
            ...
            <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 10, width: "100%" }}>
              <section>
...
function renderValue(v: unknown, depth: number): React.ReactNode {
  ...
  if (isPlainObject(v)) {
    ...
    return (
      <dl className="io-kv" style={{ display: "grid", gridTemplateColumns: "180px 1fr", gap: 6, margin: 0 }}>
        {entries.map(([k, val]) => (
          <React.Fragment key={k}>
            <dt style={{ color: "#cbd5e1", fontWeight: 500 }}>{k}</dt>
            <dd style={{ margin: 0 }}>{renderValue(val, depth + 1)}</dd>
          </React.Fragment>
        ))}
      </dl>
    );
